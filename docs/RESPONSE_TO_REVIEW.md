...
**R3-3 "重跑已完成"陈述不实 — 接受，已更正。**
本文件此前声称全量重跑完成，实际当时 n=16/17 仍在运行。**当前真实状态：
n=4..15 已用审计脚本（含逐实例模型验证）跑完并通过；n=16、17 运行中；
聚合器通过后本段将更新为"全部完成"并附 SUMMARY。** 在此之前，定理 B 的
已归档审计证据覆盖 n≤15；n=16/17 此前的非审计日志（逐阶计数与 A000109
一致、全部可行）作为过程信息保留。
